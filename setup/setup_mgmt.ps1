...
if (-not $vnet_exists) {
    ...
    # Set DNS servers
    # The Azure DNS 168.63.129.16 is required for the private endpoints to work.
    az network vnet create `
        --resource-group $MGMT_RESOURCE_GROUP_NAME `
        --name $VNET_NAME `
        --address-prefix $VNET_IP_RANGE `
        --location $LOCATION `
        --dns-servers 168.63.129.16 10.250.255.4 10.250.255.5 `
        --only-show-errors `
        --output none
}
else {
...
if ($webServiceEndpoints -notcontains "Microsoft.KeyVault" -or $webServiceEndpoints -notcontains "Microsoft.Storage" -or $webServiceEndpoints -notcontains "Microsoft.ContainerRegistry") {
...
if ($appServiceEndpoints -notcontains "Microsoft.KeyVault" -or $appServiceEndpoints -notcontains "Microsoft.Storage" -or $appServiceEndpoints -notcontains "Microsoft.ContainerRegistry") {
...
if ($mgmtServiceEndpoints -notcontains "Microsoft.KeyVault" -or $mgmtServiceEndpoints -notcontains "Microsoft.Storage" -or $mgmtServiceEndpoints -notcontains "Microsoft.ContainerRegistry") {
