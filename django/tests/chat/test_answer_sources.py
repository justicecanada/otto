...
def test_answer_sources(client, all_apps_user, load_example_pdf):
    ...
    with patch(
        "llama_index.embeddings.openai.base.get_embedding", return_value=[0.0] * 1536
    ):
        user = all_apps_user()
        llm = OttoLLM()
        ...
        example_document = load_example_pdf
        ...
        retriever = llm.get_retriever(
            ...
            filters=MetadataFilters(
                filters=[
                    MetadataFilter(key="node_type", value="document", operator="!="),
                    MetadataFilter(key="source", value=example_document.filename),
                ]
            ),
        )
        source_nodes = retriever.retrieve(query)
