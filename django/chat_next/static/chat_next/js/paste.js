...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var textPasteGuardUntil = 0;

    var stopPasteEvent = function (event) {
    ...
    var hasTextLikeClipboardTypes = function (clipboardData) {
      if (!clipboardData) return false;

      if (clipboardData.types && clipboardData.types.length) {
        for (var i = 0; i < clipboardData.types.length; i++) {
          var typeName = clipboardData.types[i];
          if (typeof typeName !== 'string') continue;
          if (typeName.indexOf('text/') === 0 || typeName === 'text/html' || typeName === 'text/rtf') {
            return true;
          }
        }
      }
    ...
    var rollbackHijackedImageUpload = function () {
      var now = Date.now();
      if (now > textPasteGuardUntil) return;
      ...
      for (var i = 0; i < fileInput.files.length; i++) {
        var mime = fileInput.files[i].type || '';
      ...
      // Clear pending image upload if it originated from a text-like paste.
      if (typeof window.clearPromptUploadedFiles === 'function') {
        window.clearPromptUploadedFiles();
      } else {
        fileInput.value = null;
      }
    };

    var setTextPasteGuard = function () {
      textPasteGuardUntil = Date.now() + 1500;
      setTimeout(rollbackHijackedImageUpload, 0);
      setTimeout(rollbackHijackedImageUpload, 60);
      setTimeout(rollbackHijackedImageUpload, 250);
    };
    ...
    if (chatPrompt) {
      chatPrompt.addEventListener('paste', function (event) {
        ...
        // Spreadsheet-specific and alternate text clipboard flavors
        var tabData = clipboardData.getData('text/tab-separated-values');
        var csvData = clipboardData.getData('text/csv');
        var rtfData = clipboardData.getData('text/rtf');

        // Fallback text payload used when text/plain is unavailable
        var fallbackText = plainData || tabData || csvData;

        // Some apps expose text clipboard flavors via items/types while getData('text/plain') is blank.
        var hasStringClipboardType = false;
        if (clipboardData.items && clipboardData.items.length) {
          for (var itemIndex = 0; itemIndex < clipboardData.items.length; itemIndex++) {
            var item = clipboardData.items[itemIndex];
            if (item && item.kind === 'string') {
              hasStringClipboardType = true;
              break;
            }
          }
        }
        if (!hasStringClipboardType && clipboardData.types && clipboardData.types.length) {
          for (var typeIndex = 0; typeIndex < clipboardData.types.length; typeIndex++) {
            var typeName = clipboardData.types[typeIndex];
            if (typeof typeName === 'string' && typeName.indexOf('text/') === 0) {
              hasStringClipboardType = true;
              break;
            }
          }
        ...
        if (hasFiles) {
          ...
          if (!onlyImages) {
            var nonImageFileInput = document.querySelector('#id_chat-input_file');
            if (nonImageFileInput) {
              stopPasteEvent(event);
              nonImageFileInput.files = clipboardData.files;
              nonImageFileInput.dispatchEvent(new Event('change'));
              return;
            }
          }
        }
        ...
        if (!isOfficeHtml && htmlData && htmlData.trim() !== '' && isCodeLike(htmlData, fallbackText)) {
          setTextPasteGuard();
          stopPasteEvent(event);
          insert(chatPrompt, fallbackText || '');
          return;
        }

        // 4. Rich HTML content — convert to markdown.
        // When clipboard has both images and HTML (e.g. Excel/OneNote/Outlook),
        // always prefer HTML content over image fallback.
        if (htmlData && htmlData.trim() !== '' && (isOfficeHtml || hasFiles || isRichContent(htmlData, fallbackText))) {
          setTextPasteGuard();
          stopPasteEvent(event);
        ...
        // 5. Plain/spreadsheet text clipboard data — insert directly and block competing handlers.
        ...
        // If clipboard advertises text-like content but getData is empty, do not allow
        // image-upload handlers to hijack this paste.
        if (hasStringClipboardType || (rtfData && rtfData.trim() !== '') || hasTextLikeClipboardTypes(clipboardData)) {
          setTextPasteGuard();
          stopPasteEvent(event);
          return;
        }
